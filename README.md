...
There's also an app with a user interface to go with it, mostly for debugging, but also gets you instructions how to use it, and the About box and so forth.  The app lets you look at the raw XML feed from the NWS, for example.  That might eventually go away now that it's mostly working.
